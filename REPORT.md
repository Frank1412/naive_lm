...
Train the same small causal Transformer on **intentionally skewed** addition data \((a,b)\) with \(a,b \in [1,100]\), then measure validation and test metrics. The point is to show how **sampling bias** in the corpus shapes what the model sees and how well it fits under teacher forcing and greedy completion.
...
|-----------|--------|---------|
| `bias_mode` | `small_operands` | With probability `bias_mix`, draw both \(a\) and \(b\) uniformly from \([1, \texttt{bias\_region\_max}]\); otherwise draw each operand uniformly from \([1,100]\). |
...
|-----------|--------|---------|
| Fraction with both \(a \le 35\) and \(b \le 35\) | **0.245** | Lower than 88% because (i) many biased draws collide when deduplicating, and (ii) the sampler must fill the set with extra full-range pairs. |
| Fraction with \(a \le 35\) | **0.438** | Asymmetric marginal: first operand more often small. |
...
- **Architecture:** `TinyCausalTransformer` (2 layers, \(d=64\), 4 heads, FFN 128, max length 32), next-token cross-entropy with padding ignored.
...
- Training and test both come from the **same biased generative process**, so metrics are **in-distribution**. They do **not** measure fairness on a uniform \([1,100]^2\) reference distribution.
- To study **mismatch**, you would train on the biased file and evaluate on a **uniform** `test.txt` (or stratify by operand size). That would typically show weaker performance on large-\(a\), large-\(b\) cases if the model overfits the small-operand regime.
